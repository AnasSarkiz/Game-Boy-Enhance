...
The supplier import marks AGND as requiring ground and NC as do-not-connect, but it does not declare required supply pins or ground requirements for EPAD. The build retains its `source_no_power_pin_defined_warning`; power checking is incomplete. The inspection fixture's other 127 pins are unwired because it only exercises the package and metadata. It must never be used as a functional CPU schematic. Add independently verified supply-domain and connection requirements when implementing the console.
