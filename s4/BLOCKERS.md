...
**Do not fabricate. The connected placement board is incomplete and unrouted.** The 145-component power/CPU/USB/storage/controls/audio draft now uses the unchanged S3 supplier package with S4 procurement metadata, as explicitly authorized on 2026-09-06. Package and electrical qualification remain required.
...
| S4-05 | Reference pin conflicts are addressed in the 69-signal pin-allocation table: board IDs move to PE8/PE9 and UART4 is reserved on PE4/PE5. The ten PE game inputs are connected; analog audio and its enable are now connected; display/HDMI audio/debug/USB1 reservations remain unwired. | Implement the remaining reserved interfaces and matching S4 firmware. Verify the complete pinmux on hardware. |
...
All 35 candidate imports pass pad-count, courtyard-containment and conservative pad-overlap checks. The customized CPU now requires all 20 supply inputs, both LDO outputs and both ground pins to be connected, while forbidding NC106 connections. The connected stage satisfies these requirements; the isolated CPU fixture deliberately fails with 22 missing-connection errors to exercise the checks. Exact S4 voltage tolerances are not invented.
...
| S4-11 | Automatic schematic packing failed because positions nested inside sheets were not considered by the root layout selection. Sheet border rendering also ignores the recorded sheet center. | Addressed for this design using the supported explicit relative layout and centered content. All seven pages are rendered separately and reviewed. No DRC is suppressed; automatic packing remains a tool limitation. |
| S4-12 | RK10J12R0A0B/C351175 is found and imported, but its separated mounting pads repeat terminal numbers 6/7. Core emits two `source_ambiguous_port_reference` records. Its passive dual-gang chip representation also emits `source_no_power_pin_defined_warning`. | Add correct passive multi-gang and repeated mechanical-terminal support upstream. Preserve supplier geometry; no fake power pin, mounting-pad deletion or suppressed diagnostics. The acceptance audit fails and this draft is not published. |

The 240-row `reports/enhance-retention.csv` records every original component, including all pending reviews. The 33-part audio addition retains 20 original designators with documented changes. Registry v1.0.3 remains the 112-component baseline.
